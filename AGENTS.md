...
- **`mt`** owns scheduling, persistence, deterministic reuse of
...
`mt next` decides *what* to present; you decide *how*.

## Starting a session

Ask the user what they want to learn. Translate the goal into a list
of target atom IDs (leaf concepts in the curriculum, e.g.
`fnd.1.1.5`, `la.5.4.7`). If you don't know which IDs apply, ask the
user.

Start a learning path:

    mt new "Understand SVD" --atom la.5.4.1 --atom la.5.4.7
...
- `error: no learning path found` → the user has no active path.
  Run `mt new` first.
- `error: unknown atom id: X` → that atom isn't in the curriculum.
