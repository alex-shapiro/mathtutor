...
If the list is empty, ask the user what they want to learn, translate the goal into target IDs — an atom, cluster, or area root (browse with `get_children` / `get_item`) — then call `new_path { goal, targets, strategy? }`. Targets are stored as given and re-expanded on every load, so a cluster or area target keeps tracking the curriculum as it grows.
