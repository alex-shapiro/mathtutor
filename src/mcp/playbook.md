...
If the list is empty, ask the user what they want to learn, translate the goal into target IDs — atoms, clusters, or whole areas (browse with `get_children` / `get_item`) — then call `new_path { goal, targets, strategy? }`.
