...
If the list is empty, ask the user what they want to learn, translate the goal into target IDs at any level — e.g. `tx`, `tx.3`, `tx.3.2` (browse with `get_children` / `get_item`) — then call `new_path { goal, targets, strategy? }`.
