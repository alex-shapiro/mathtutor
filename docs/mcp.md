...
- Content: A structured object containing the raw data (IDs, statuses, content) without decorative formatting.

#### Error Handling

If a tool fails due to business logic (e.g., "atom not found", "invalid rating"), the server returns a `CallToolResult` with `isError: true`. This allows the LLM to see the error message in its context and attempt to fix its call.
...
The MCP response content includes both the standard output and any relevant error messages (stderr) generated by the underlying logic.
